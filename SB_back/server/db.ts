...
import { users, profiles, preferences, favorites, InsertUser, InsertProfile, InsertPreference, User, Profile, Preference, Favorite } from "../drizzle/schema";
 ...
 */
export async function createUser(email: string, passwordHash: string, telegramUsername?: string): Promise<User | null> {
 ...
 */
export async function upsertProfile(userId: number, data: Omit<InsertProfile, 'userId'>): Promise<Profile | null> {
  const db = await getDb();
  if (!db) {
 ...
 */
export async function upsertPreferences(userId: number, data: Omit<InsertPreference, 'userId'>): Promise<Preference | null> {
  const db = await getDb();
  if (!db) {
...
export async function getUserFavorites(userId: number): Promise<number[]> {
  ...
  return result.map(f => f.favoriteUserId);
}

/**
 * Get users who favorited this user (admirers)
 */
export async function getUserAdmirers(userId: number): Promise<User[]> {
  ...
  const admirers = await db.select().from(users).where(
    inArray(users.id, admirerIds.map(a => a.userId))
  );
  return admirers;
}
 ...
 */
export async function getAllProfiles(): Promise<(Profile & { user: User })[]> {
  const db = await getDb();
  if (!db) {
